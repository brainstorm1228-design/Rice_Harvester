...
현재 기준 릴리스 단계는 **2.0.0 Alpha**이며, main 브랜치에 기록되는 제품 버전은 **2.0.0**입니다.

- 대규모 버전업: `1.0.0` 단위 증가
- 소소한 기능 추가: `0.1.0` 단위 증가
- 버그 수정: `0.0.1` 단위 증가
- main 브랜치의 설치파일/제품 버전 문자열에는 `alpha` 접미사를 붙이지 않습니다.
- Alpha/Beta/Stable 같은 단계 표기는 문서와 릴리스 노트에서 관리합니다.

## 2.0.0 Alpha 방향

2.0.0 Alpha는 확대 모니터링 화면을 단순 보기 창에서 실제 작업 화면으로 확장하는 버전입니다.

- 확대화면 우측에 워크플로우 작업 패널 추가
- 확대화면에서 화면 좌표를 클릭해 클릭/이동 단계를 바로 추가
- 확대화면에서 작성 중인 워크플로우를 대상 PC 또는 미러링 대상 PC에 실행
- 작성 중인 워크플로우를 현재 PC에서 테스트 실행
- Debug Agent의 입력 테스트를 no-op이 아닌 로컬 Windows 입력 실행으로 전환
- 모니터링 5대 이상/9대 이상 상황에서 타일 렌더링 속도를 자동 제한해 Controller UI 부하 완화
...
- 확대 창 제어
  ...
  - 우측 워크플로우 작업 패널에서 좌표 기반 단계 작성
  - 작성 중인 단계는 대상 PC 실행, 미러링 실행, 현재 PC 테스트 실행 가능
- 워크플로우
...
- Debug 모드
  ...
  - 현재 PC 테스트 실행 시 Windows 로컬 입력으로 워크플로우 검증 가능
- 입력 백엔드
